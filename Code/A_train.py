...
def diffusion_train(configs):
    ...
    for epoch in range(configs.epoch_diff):
        ...
        for observed_data, observed_dataf, observed_mask, observed_tp, gt_mask in tqdm(
            train_loader, desc=f"Train {epoch+1}/{configs.epoch_diff}", leave=False
        ):
            iter_count += 1
            model_optim.zero_grad()
            observed_data = observed_data.to(configs.device)
            observed_dataf = observed_dataf.to(configs.device)
...
def diffusion_test(configs, model):
    ...
    for observed_data, observed_dataf, observed_mask, observed_tp, gt_mask in tqdm(
        ...
        imputed_sample = imputed_samples.median(dim=1).values
    ...
    out_path = results_root / f"FGTI_{configs.dataset}_p{int(configs.missing_rate*100)}_seed{configs.seed}.csv"
